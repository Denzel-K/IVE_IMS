const API_BASE_URL = 'http://localhost:3500/api/equipment-sharing'; // Replace with your backend API URL
...
function initializeModals() {
    ...
    closeButtons.forEach(button => {
        button.addEventListener('click', () => {
            const modal = button.closest('.modal-content').parentElement;
            if (modal) {
...
async function fetchData(endpoint) {
    try {
        ...
        return data.data || data; // Handle both array and object responses
    } catch (error) {
...
async function renderRequestsFromLabs() {
    ...
    try {
        ...
        console.log("Requests from Labs:", requestsFromLabs); // Debugging

        // Ensure requestsFromLabs is an array
        if (!Array.isArray(requestsFromLabs)) {
            throw new Error("Expected an array of requests, but received a different data structure.");
        }
        ...
        console.error("Error rendering requests from labs:", error);
        container.innerHTML = `<p class="text-red-500">Error loading requests. Please try again later.</p>`;
    }
}
